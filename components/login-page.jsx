...
export function LoginPageComponent() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('https://35.239.105.236/auth/login', {
        method: 'POST',
        headers: {
  ...
  const handleVerifyEmail = async (e) => {
    ...
    try {
      const response = await axios.get(`https://35.239.105.236/auth/verify_email/send_token/${verificationEmail}`)
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://35.239.105.236/auth/verify_email/verify_token', {
