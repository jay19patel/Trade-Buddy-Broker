...
export default function Component() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await fetch('https://35.239.105.236/transaction/get_all_transactions', {
        method: 'GET',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const token  = Cookies.get("access_token")
      const response = await fetch('https://35.239.105.236/transaction/create_transaction', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
