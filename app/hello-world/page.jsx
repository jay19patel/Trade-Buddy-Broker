...
export default function ContactPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('https://35.239.105.236/auth/help_message_send', {
        method: 'POST',
        headers: {
    ...
    <div className="container mx-auto px-4 py-8 bg-gradient-to-br from-purple-100 to-blue-100 min-h-screen">
      ...
      <Card className="mt-8 bg-white shadow-lg">
        <CardHeader className="bg-green-600 text-white">
          ...
          <a href={"https://35.239.105.236/docs"} className="text-white hover:underline mt-2 inline-block">Learn more</a>
        </CardHeader>
        <CardContent className="mt-4">
          <ul className="space-y-4">
            {[
              { icon: User, title: "User Management API", description: "Manage user accounts and authentication.", link: "https://35.239.105.236/docs" },
            ].map((api, index) => (
