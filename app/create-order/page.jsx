...
export default function StockSearchApp() {
  ...
  const fetchSearchResults = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`https://35.239.105.236/live_data/search?q=${searchTerm}`)
  ...
  const handleStockSelect = async (stock) => {
    ...
    try {
      ...
      const response = await fetch(`https://35.239.105.236/live_data/find?type_of_symbol=${stock.entity_type}&symbol_id=${symbolId}`)
  ...
  const addOrder = async (orderDetails) => {
    ...
    try {
      ...
      const response = await fetch('https://35.239.105.236/order/new_order/', {
        method: 'POST',
